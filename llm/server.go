...
func (s *llamaServer) Load(ctx context.Context, gpus discover.GpuInfoList, requireFull bool) error {
	...
	// TODO: Update Library based on changed NumGPU?
...
func (s *ollamaServer) Load(ctx context.Context, gpus discover.GpuInfoList, requireFull bool) error {
	// TODO: If we have a previous allocation still in progress and we updated the free memory,
	// the model will now be double counted
	...
	// TODO: ggml_new_object: not enough space in the context's memory pool (needed 1104, available 928)
	// ggml.c:1569: GGML_ASSERT(obj_new) failed
	...
	for {
		req.GPULayers = gpuLayers
		resp, err := s.initModel(ctx, req, false)
		if err != nil {
			return err
		}
	...
	resp, err := s.initModel(ctx, req, true)
	if err != nil {
		return err
	}
...
func distributeGPU(layers []uint64, gpus discover.GpuInfoList, requestedLayers int, forceRequest bool) (gpuLayers ml.GPULayersList) {
	...
	bestAssignemnts := fit(layers, gpus, high, requestedLayers)
	maxNumGPU := bestAssignemnts.Sum()
	if maxNumGPU == 0 {
		return bestAssignemnts
	}

	for high-low > 1e-6 {
		mid := (low + high) / 2
		assignemnts := fit(layers, gpus, mid, requestedLayers)
		if assignemnts.Sum() == maxNumGPU {
			high = mid
...
nextLayer:
	...
	if requireFull {
		if gpuLayers.Sum() < len(layers) && gpuLayers.Sum() < s.options.NumGPU && !(len(gpus) == 1 && gpus[0].Library == "cpu") {
...
func (s *llmServer) WaitUntilRunning(ctx context.Context) error {
	// TODO: We should capture the entire loading time, including allocations
	start := time.Now()
